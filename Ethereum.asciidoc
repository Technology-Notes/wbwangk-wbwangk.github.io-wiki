...
### Ganache
Ganache是一个以太坊的本地开发环境，可以模拟出智能合约的运行环境。[1](http://truffleframework.com/docs/ganache/using) Ganache提供图形界面，支持Windows、MacOS、Linux。对于没有图形界面的服务器环境，使用ganache-cli（以前叫testrpc）[2](https://github.com/trufflesuite/ganache-cli/blob/master/README.md) 。  
...
**注意：对于以太坊的基础知识，请在继续之前阅读Truffle的[以太坊概述](https://github.com/wbwangk/wbwangk.github.io/wiki/Ethereum#以太坊概述)。**
