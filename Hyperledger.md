...
#### 调用(invoke)
用于调用链码函数。一个客户端应用通过发送事务提议到一个peer来调用链码。peer会执行链码和返回一个背书提议响应到客户端应用。客户端应用收集足够的提议响应以便满足背书策略，然后递交事务结果以便排序、验证和提交。客户端应用程序也可以选择不去递交事务结果。例如查询账本，客户端应用程序通常不会递交这个只读事务，除非是为了审计的目的想把读账本的行为记录日志。调用包括通道id、调用的链码函数和参数数组。  
