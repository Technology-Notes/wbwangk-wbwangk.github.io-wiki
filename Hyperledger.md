...
#### 背书策略
定义了通道中的哪些peer节点必须执行附加在链码应用中的事务，和需要的响应(背书)组合。需要一个策略，定义一个事务至少得到几个peer的同意，或最少多少比例的peer同意，或全部peer的同意，这个策略会附加到一个特定链码应用上。策略可以根据应用程序和期望水平来反对不当行为（有意或无意）。一个事务在被提交peer标记已验证前，必须符合背书策略。对部署、实例化事务也需要一个明确的背书政策。  
