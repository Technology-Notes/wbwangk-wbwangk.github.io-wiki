...
到[Oracle Java下载页面](http://www.oracle.com/technetwork/java/javase/downloads/index.html)下载RPM包。直接用wget不行，先下载到windows下，然后进git bash，然后用scp复制到centos中(scp jdk-8u131-linux-i586.rpm root@c6801:/opt)。
```
$ rpm -ivh jdk-8u131-linux-i586.rpm  或  yum install jdk-8u131-linux-i586.rpm
```