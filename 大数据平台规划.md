...
#### NameServer
各个节点的/etc/resolv.conf要添加统一的nameserver，代替hosts文件中域名配置。  

#### 认证
用户凭据无疑要放在LDAP中。    
knoxsso(jwt令牌)充当UI/REST认证。  
kerberos+SPNEGO充当命令行、程序调用、JDBC等的认证。  
kerberos网关，是FreeIPA内置功能，充当互联网用户kerberos+SPNEGO认证。或直接使用KDCProxy搭建。    、
