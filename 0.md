...
4. Customs entity is validated and enabled to perform a “counterfeit check”
5. Customs submits a “counterfeit check” request
6. Submitting client starts the “counterfeit check” via counterfeit recognition processing
  based on the received microchips and associated data
7. Endorsing peer enacts a ”counterfeit check” via Chain Code to validate product
  provenance and authenticity status
8. Chain code interacts with the Data Validation Layer to validate the microchip’s status
9. A possible counterfeit product is identified as the item has arrived in a San Diego,
  California Port from the APAC region, expected arrival Port was Miami, Florida from the
  EMEA region
10. Read/write set (possible counterfeit product) is delivered to the submitting client
11. A “counterfeit notification” transaction is endorsed and delivered
12. “Counterfeit notification” transaction delivered
13. “Counterfeit notification” data is stored on the ledger.
...
4. Have globally recognized identification.
5. Be uniquely identified.
6. 3
  3
...
7. Must be globally identifiable.
8. Must be uniquely identifiable.
9. 3Must have items identification applicable to all product hierarchy levels as
  appropriate.
10. Must be assigned at the latest when physically created.
11. Must at a minimum have GTIN identification for trade items.
12. Varying precision level required depending on the item (e.g., more precision
   spreading from batch/lot number to serial number/SGTIN).
13. Identification precision level defined by the traceability partner.
14. Logistics units must be uniquely identified (e.g., serial shipping container code
   specifications).
15. All traceable items must carry identifiers on the asset containing it or accompanying
   documents.
16. Best practices dictate using a human readable form in the most appropriate location
   (e.g. on the product or with the product documentation etc.).
17. Identification carrier must remain on or attached to the traceable item when packed in
   an upper level of packaging.
   Section 4 - External References and Glossary
   Cited Quotes:
18. http://www.businessinsider.com/technology-is-a-huge-national-security-threat-2013-8
19. https://www.unglobalcompact.org/docs/issues_doc/supply_chain/Traceability/Guide_to_
   Traceability.pdf
20. https://knowthechain.org/the-logistics-of-tracking-traceability/
21. https://www.semiconductors.org/clientuploads/directory/DocumentSIA/Anti
   Counterfeiting Task Force/ACTF Whitepaper Counterfeit One Pager Final.pdf
22. https://hbr.org/2017/03/global-supply-chains-are-about-to-get-better-thanks-toblockchain
23. http://www.gs1.org/sites/default/files/docs/traceability/Global_Traceability_Standard.pdf
